...
router.get("/guides/:guideId", async (req, res) => {
    try {
        ...
        return (0, response_1.sendSuccess)(res, 200, "Guide details retrieved", {
            guideId: guide.guide_id,
            name: guide.users.full_name,
            email: guide.users.email,
            phone: guide.users.phone,
        ...
        return (0, response_1.sendError)(res, 500, "Failed to get guide details");
    }
});
/**
 * GET /hotels
...
router.get("/hotels", async (req, res) => {
    try {
        ...
        const total = await prisma_1.prisma.hotel.count({ where: { verified_status: true } });
...
router.get("/hotels/:hotelId", async (req, res) => {
    try {
        ...
        return (0, response_1.sendSuccess)(res, 200, "Hotel details retrieved", {
            hotelId: hotel.hotel_id,
            name: hotel.hotel_name,
            location: hotel.location,
            description: hotel.description,
        ...
        return (0, response_1.sendError)(res, 500, "Failed to get hotel details");
    }
});
/**
 * GET /search
...
router.get("/search", async (req, res) => {
    try {
        ...
        if (!searchQuery) {
            return (0, response_1.sendError)(res, 400, "Search query required");
        }
        ...
        if (type === "guides" || type === "all") {
            const guideResults = await prisma_1.prisma.guide.findMany({
        ...
        if (type === "hotels" || type === "all") {
            hotels = await prisma_1.prisma.hotel.findMany({
                where: {
                    verified_status: true,
                    OR: [
        ...
        if (type === "all") {
            destinations = await prisma_1.prisma.destination.findMany({
                where: {
                    OR: [
        ...
        return (0, response_1.sendSuccess)(res, 200, "Search results", {
            guides,
            hotels: hotels.map((hotel) => ({
                type: "hotel",
                id: hotel.hotel_id,
        ...
        return (0, response_1.sendError)(res, 500, "Search failed");
    }
});

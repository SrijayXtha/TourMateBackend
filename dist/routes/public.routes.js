...
router.get("/guides", async (req, res) => {
    try {
        ...
        const total = await prisma_1.prisma.guide.count({
        ...
        return (0, response_1.sendSuccess)(res, 200, "Guides retrieved", {
            guides: guides.map((g) => ({
                ...
                avgRating: g.review.length > 0
                    ? (g.review.reduce((sum, r) => sum + (r.rating || 0), 0) / g.review.length).toFixed(2)
                    : "N/A",
    ...
    try {
        ...
        return (0, response_1.sendSuccess)(res, 200, "Hotels retrieved", {
            hotels: hotels.map((h) => ({
                ...
                avgRating: h.review.length > 0
                    ? (h.review.reduce((sum, r) => sum + (r.rating || 0), 0) / h.review.length).toFixed(2)
                    : "N/A",
...
router.get("/search", async (req, res) => {
    try {
        ...
        if (type === "guides" || type === "all") {
            guides = await prisma_1.prisma.guide.findMany({
                where: {
                    verified_status: true,
                    OR: [
                        {
